  ...
  useEffect(() => {
    const fetchVestingData = async () => {
      ...
      try {
        ...
        const address = employeeVestingAddress;
        const abi = employeeVestingABI;

        if (!address || address.includes('0x...')) {
            console.error(`Contract address for Employee vesting is not configured.`);
            alert(`Contract address for Employee vesting is not configured. Please set it in your environment variables.`);
            setVestingData([]);
            return;
        }
      ...
      <div>
        <div className="flex justify-between items-center mb-4 p-6">
          <div className="font-['Nunito:Bold',_sans-serif] font-bold text-[#030229] text-[24px]">
            <p>Employee Vesting Release List (Vesting Status)</p>
          </div>
