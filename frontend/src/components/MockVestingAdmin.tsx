  ...
  useEffect(() => {
    const fetchVestingData = async () => {
      ...
      try {
        ...
        const address = mockVestingAddress;
        const abi = mockVestingABI;

        console.log("Contract Address:", address);
        console.log("Contract ABI:", abi);

        if (!address || address.includes('0x...')) {
            console.error(`Contract address for Mock vesting is not configured.`);
            alert(`Contract address for Mock vesting is not configured. Please set it in your environment variables.`);
            setVestingData([]);
            return;
        }
        ...
        const schedules = await Promise.all(beneficiaries.map(async (beneficiary: string) => {
          console.log(`Fetching schedule for beneficiary: ${beneficiary}`);
